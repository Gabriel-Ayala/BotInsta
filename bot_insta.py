...
class InstagramBot:
    ...
    def login(self,hashtag):
        ...
        self.curtir_fotos_com_a_hastag(hashtag)
    ...
    def curtir_fotos_com_a_hastag(self, hashtag):
        ...
        for pic_href in pic_hrefs:
            ...
            driver.execute_script(
                "window.scrollTo(0, document.body.scrollHeight);")
            try:
                #driver.find_element_by_xpath("//button[@class="wpO6b _8-yf5"]').click()
                driver.execute_script('document.getElementsByClassName("wpO6b", "QBdPu")[1].click()')
                time.sleep(random.randint(19, 23))
            except Exception as e:
                print(e)
                time.sleep(5)
with open('hashtag.txt', 'r') as hashtag:
    with open('username.txt', 'r') as username:
        with open('password.txt', 'r') as password:
            tag = str(hashtag.read())
            user = str(username.read())
            passw = str(password.read())

            Bot = InstagramBot(user,passw)
            Bot.login(hashtag)
